...
void DiskManager::destroy_file(const std::string &path) {
    ...
    if (unlink(path.c_str()) == -1) {
        throw std::runtime_error("DiskManager::destroy_file Error: failed to delete file");
    }
}
...
void DiskManager::close_file(int fd) {
    ...
    // delete map entry
    auto dpath = path2fd_.find(this->get_file_name(fd));
    if (dpath != path2fd_.end()) {
        path2fd_.erase(dpath);
    }

    auto dfd = fd2path_.find(fd);
    if (dfd != fd2path_.end()) {
        fd2path_.erase(dfd);
    }
}
